...
def get_movie_critics_for_letter(letter):
    url = 'http://www.metacritic.com/movie/' + letter + '/critic-reviews'
    ...
    critics_elements = soup.find_all('a', {'class': 'no_hover'})

    critics = []
    for critic_element in critics_elements:
        link_element = critic_element.getText()
        link_element = link_element.encode('utf-8').decode('ascii', 'ignore')
        critics.append(str(link_element).strip())
